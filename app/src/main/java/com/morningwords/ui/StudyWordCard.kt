...
import com.morningwords.speech.label
...
internal fun StudyWordCard(
    ...
    onSpeak: (EnglishAccent) -> Unit,
    modifier: Modifier = Modifier,
    currentAccent: EnglishAccent = EnglishAccent.US,
) {
    ...
    Surface(shape = RoundedCornerShape(28.dp), color = MaterialTheme.colorScheme.surface,
        ...
        Column(Modifier.fillMaxSize()) {
            Text("当前：${currentAccent.label}", modifier = Modifier.align(Alignment.End).padding(end = 24.dp, top = 16.dp), color = sage, style = MaterialTheme.typography.labelLarge)
            Column(Modifier.weight(1f).fillMaxWidth().verticalScroll(rememberScrollState()).padding(horizontal = 24.dp, vertical = 16.dp),
                ...
                PronunciationButtons(pronunciationReady, onSpeak, Modifier.padding(top = 8.dp))
                if (!pronunciationReady) Text("正在准备发音…", color = StitchMuted, style = MaterialTheme.typography.bodySmall)
                if (answerShown) {
